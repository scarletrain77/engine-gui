...
var engine;
(function (engine) {
    class listview extends engine.DisplayObjectContainer {
        constructor() {
            super();
            this.displayList = [];
        }
        updateView() {
            ...
            for (let i = 0; i < this.datas.length; i++) {
                ...
                newView.y = i * height;
                //将data赋值给copiedView
                this.assignmentFunc(data, newView);
                this.displayList.push(newView);
                this.addChild(newView);
            }
        }
    }
    engine.listview = listview;
})(engine || (engine = {}));
var engine;
(function (engine) {
    class Context {
            ...
            window.onmouseup = (up) => {
                ...
                try {
                    if (downChain[downChain.length - 1].id == upChain[upChain.length - 1].id) {
                        ...
                        stage.dispatchEvent("capture", downChain, clickEvent);
                        stage.dispatchEvent("bubble", downChain, clickEvent);
                    }
                }
                catch (e) { }
            };
...
(function (engine) {
    var RES;
    (function (RES) {
        var RESOURCE_PATH = "./resources/"; //todo should be "././Resources/"
